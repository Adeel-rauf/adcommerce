...
const ListProduct = ({ product, bg }: Props) => {
  return (
    <div className="w-full relative group flex items-center border-[1px] hover:shadow-lg duration-200 shadow-gray-500 rounded-md overflow-hidden group">
      <div className="max-w-80 max-h-80 flex">
        ...
        {product?.isnew && (
          <div className="absolute top-2 right-2 z-50">
            <p className="bg-gray-500 px-4 py-1 text-white flex justify-center items-center text-sm 
            font-semibold hover:bg-black duration-300 cursor-pointer rounded-md">
              New
            </p>
          </div>
        )}
      ...
      <div className="py-6 flex flex-col gap-5 px-4">
        ...
        <div className="flex items-center gap-2">
         ...
         <p className="text-[#767676] text-xs line-through">
          ...
          <p className="font-semibold">${product?.rowprice}</p>
          </>
           :
           <>
        ...
        <div className="flex items-center gap-5 duration-300">
          <button
           className="bg-gray-800 text-gray-200 px-4 py-2 text-xs rounded-full flex items-center 
          gap-1 hover:bg-gray-950 hover:text-white duration-200">
      ...
      {/* {product.isnew? 
      <span className="absolute right-3 top-5 bg-black text-white px-2 py-1 rounded-lg">New</span>:''
      } */}
    </div>
  );
};
